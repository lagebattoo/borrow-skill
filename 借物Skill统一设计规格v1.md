...
正文完整保留用户提供的《「借物」Skill 统一设计规格 v1》。本附录记录命名、文档关系、文件拆分及落地进度；不改写正文。文件编写、结构检查与实际 Agent 行为验证分别记录，不互相替代。
...
| 1. 定位与边界 | 已形成设计规格 |
| 2. 触发条件与输入输出 | 已形成设计规格 |
| 3. 执行流程与决策规则 | 已形成设计规格，执行细节已写入初稿，见附录 D |
| 4. 编写 Skill 文件 | 已编写入口、4 份参考规则、2 份模板及展示配置，共 8 个文件 |
| 5. 案例验证与调整 | 格式与引用检查通过；实际 Agent 案例验证尚未执行 |
...
以下结构已落地为 Skill 初稿，另增加 `agents/openai.yaml` 保存已确认展示名和调用提示：
...
`borrow/agents/openai.yaml` 仅提供展示名、简介和调用提示；保留默认自动发现，不添加额外工具依赖。

## D. 执行细节的初稿处理（实现解释，待案例验证）

下列处理已写入 Skill 初稿，属于对规格的实现解释，不混入用户原始正文，也不宣称已通过实际行为验证：

1. 缺失信息影响目标、硬性筛选或高成本架构选择时追问；低影响假设明确标注，其他独立分析继续，受影响的决定保持 OPEN。
2. 无法联网或证据不足时标记未验证，使用 OPEN 或有条件的 PREFERENCE；不把未知当作检查通过，也不以搜索不可用推断无候选而选择 BUILD。
3. 目标环境没有 search-first 时，将同等的实现阶段检查交给实际执行 Agent，不强制安装额外 Skill，不宣称 search-first 已运行。

后续应通过案例观察这些处理是否有效，再据证据调整。

## E. 验收状态与后续范围

已运行 skill-creator 的 `quick_validate.py`，结果为 `Skill is valid!`。额外检查了 8 个 Skill 内部相对链接、展示配置、默认自动发现设置及两份用户原始正文的保留情况，均通过。校验所需 PyYAML 放在项目外的临时目录，不是 Skill 的运行依赖。

后续仍需用实际 Agent 案例验证触发范围、复用决策和交接行为。案例应覆盖小修改不误触发、简单功能不堆积依赖、核心差异保持项目掌控，以及复用预算不放宽硬性筛选。Skill 尚未安装，也未宣称实际触发或行为测试通过。
